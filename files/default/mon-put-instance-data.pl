...
Description of available options:
  ...
  --service-avail     Reports service available.
  --service-name=SERVICES Selects the services by name on which to report.
  --command-avail     Reports command available.
  --command-name=COMMANDS Selects the command by name on which to report.
...
my $report_service_avail;
my $report_command_avail;
...

{
  ...
  $parse_result = GetOptions(
    ...
    'service-avail' => \$report_service_avail,
    'service-name:s' => \@service_name,
    'command-avail' => \$report_command_avail,
    'command-name:s' => \@command_name,
...
foreach my $service (@service_name) {
  if (length($service) == 0) {
    exit_with_error("Name of service is not specified.");
  }
  else {
    $report_service_avail = 1;
  }
}

foreach my $command (@command_name) {
  if (length($command) == 0) {
    exit_with_error("Name of command is not specified.");
  }
  else {
    $report_service_avail = 1;
  }
...
if (!$report_mem_util && !$report_mem_used && !$report_mem_avail
  && !$report_swap_util && !$report_swap_used && !$report_disk_space && !$report_service_avail
  && !$report_command_avail)
{
...
if ($report_service_avail)
{
  foreach my $line (@service_name)
  {
    my $service = `PATH=/usr/sbin:/usr/bin:/sbin:/bin service $line status`;
    if (index ($service,'is running')>=0 || index ($service,'start/running')>=0) {
      add_metric('ServiceStatus_'.$line, 'Count', 1);
    }
    else {
      add_metric('ServiceStatus_'.$line, 'Count', 0);
    }
  }
}

# collect custom command  metric

if ($report_command_avail)
{
  foreach my $line (@command_name)
  {
    my $command = `PATH=/usr/sbin:/usr/bin:/sbin:/bin sh /etc/aws-scripts-mon/custom-scripts/$line `;
    if (index ($command,'OK')>=0) {
      add_metric('CommandStatus_'.$line, 'Count', 1);
    }
    else {
      add_metric('CommandStatus_'.$line, 'Count', 0);
    }
  }
}
